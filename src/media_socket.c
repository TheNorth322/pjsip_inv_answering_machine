    ...
    pj_pool_t *pool, pjmedia_endpt *endpt, const pj_uint16_t af, const int rtp_port, struct media_socket_t **socket)
{
    ...
    status = pjmedia_transport_udp_create3(endpt, af, NULL, NULL, rtp_port, 0, &(*socket)->med_transport);
    ...
    pj_memcpy(&(*socket)->sock_info, &(*socket)->med_tpinfo.sock_info, sizeof(pjmedia_sock_info));
